# Agent Guidelines for Foundatio.Minio
...
```bash
# Build
dotnet build Foundatio.Minio.slnx

# Test
dotnet test Foundatio.Minio.slnx

# Format code
dotnet format Foundatio.Minio.slnx
```
...
1. **Builds successfully**: `dotnet build Foundatio.Minio.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.Minio.slnx` shows no failures
...
- Mirror the main code structure (e.g., `Storage/` tests for `src/Foundatio.Minio/Storage/`)
...
# Specific test file
dotnet test --filter "FullyQualifiedName~MinioStorageTests"
